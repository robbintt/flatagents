...
# Add python root to path for local imports
...
async def cmd_generate_data(args):
    ...
    generator = DataGenerator(EXAMPLE_ROOT / "config")
    ...
    output_path = args.output or (EXAMPLE_ROOT / "data" / "evaluation_set.json")
...
async def cmd_evaluate(args):
    ...
    judge_path = args.judge or (EXAMPLE_ROOT / "config" / "agents" / "judge.yml")
    data_path = args.data or (EXAMPLE_ROOT / "data" / "evaluation_set.json")
...
async def cmd_optimize(args):
    ...
    data_path = EXAMPLE_ROOT / "data" / "evaluation_set.json"
    if not data_path.exists():
        logger.error(f"Data file not found: {data_path}")
        logger.info("Run 'python main.py generate-data' first")
        return

...
# --- Script Logic ---
echo "--- FlatAgents GCP Helloworld Demo ---"
...
# 4. Start function server in background
echo ""
...
"$VENV_PATH/bin/python" -m functions_framework --target=helloworld --port=$PORT &
SERVER_PID=$!
...
# 5. Run the test
echo ""
echo "🧪 Running test: Build 'Hi' char by char"
echo "---"
...
# 6. Validate result
if echo "$RESULT" | grep -q '"result":"Hi"'; then
    echo "✅ Test passed! Machine built 'Hi' successfully."
else
    echo "❌ Test failed. Expected result to contain 'Hi'."
    exit 1
fi

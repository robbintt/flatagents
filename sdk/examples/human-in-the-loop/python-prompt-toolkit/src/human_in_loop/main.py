"""
Human-in-the-Loop Demo for FlatAgents using prompt_toolkit.
...
This version uses prompt_toolkit for a better REPL experience
with multiline support and control code handling.
...
async def run(topic: str = "the benefits of daily exercise", max_revisions: int = 3):
    ...
    """
    logger.info("=" * 60)
    logger.info("Human-in-the-Loop Demo (FlatMachine + prompt_toolkit)")
    logger.info("=" * 60)

    # Load machine from YAML (shared config directory)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Human-in-the-loop content generation (prompt_toolkit version)"
    )
    parser.add_argument(
        "--topic",

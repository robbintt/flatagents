...
class HelloWorldHooks(LoggingHooks):
    """Hooks for the HelloWorld smolagents demo with append_char action."""
...
async def run():
    """
    Main function to run the FlatMachine HelloWorld demo with smolagents.

    The loop and state management is handled by FlatMachine,
    defined in config/machine.yml.  The agent call is delegated to a
    smolagents CodeAgent via the subprocess bridge adapter.
    """
    logger.info("--- Starting FlatMachine HelloWorld Demo (smolagents) ---")
    ...
    logger.info("Building character by character via smolagents bridge...")

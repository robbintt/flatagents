...
# --- Script Logic ---
echo "--- FlatMachine HelloWorld (smolagents) Demo Runner ---"
...
if [ "$LOCAL_INSTALL" = true ]; then
    ...
    uv pip install --python "$VENV_PATH/bin/python" smolagents
else
    echo "  - Installing from PyPI (deps from pyproject.toml)..."
fi

echo "  - Installing helloworld-smolagents demo package..."
uv pip install --python "$VENV_PATH/bin/python" -e "$SCRIPT_DIR"

# 3. Run the Demo
echo "🚀 Running demo..."
echo "---"
"$VENV_PATH/bin/python" -m helloworld_smolagents.main
echo "---"
